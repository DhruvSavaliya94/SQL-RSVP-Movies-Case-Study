...
/* Info: INFORMATION_SCHEMA provides access to database metadata, information about the MySQL server 
         such as the name of a database or table, the data type of a column, row count etc...*/
...
-- Alternate query : SELECT COUNT(*) FROM TABLE_NAME;
...
-- Above query check, if value is null then it treated as 1 else 0 and adding 0s and 1s give number of nulls
-- Country, worlwide_gross_income, languages and production_company columns have NULL values in movie table
...
-- Hear, movie is also counted if multiple countries satisfy the condition any one by like operator
SELECT 
...
/* Info: In the above query first of all grouping by movie_id and then count genre and filter record having number_of_movies is one then 
         using common table expression we just count a number of records which have only one genre.*/
...
-- Top actor is Vijay Sethupathi with average rating 8.42
